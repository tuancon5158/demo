...
import { useCadreLevelTitles } from '@/state'
...
export default defineComponent({
  ...
  setup(props) {
    const { cadreLevels } = useCadreLevelTitles()
    const options = computed(() => {
      if (props.all) return cadreLevels.value

      return cadreLevels.value.filter(item => item.value)
    })
