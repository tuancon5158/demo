  ...
  <div class="py-4 px-2 bg-light-200 rounded" @click="onRedirectUpdate">
    <div class="flex justify-start space-x-4">
      ...
      <div class="flex-1 flex flex-col space-y-1">
        <div class="font-bold">
          <span>Nhân viên:</span>
          <span>{{ changingTime.user.name }}</span>
        </div>
        <div class="font-bold">
          <span>Ngày:</span>
  ...
  setup(props) {
    ...
    const onRedirectUpdate = () => {
      if (props.changingTime.model_name === 'OT') {
        return router.push(
          `/approve-work-schedule/overtime/${props.changingTime.id}`
        )
      }

      if (props.changingTime.model_name === 'ABSENT') {
        return router.push(
          `/approve-work-schedule/absence/${props.changingTime.id}`
        )
      }

      if (props.changingTime.model_name === 'OUTSIDE') {
        return router.push(
          `/approve-work-schedule/outside/${props.changingTime.id}`
        )
      }
    }
